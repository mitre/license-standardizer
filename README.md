...
REPO_MINDER_AI_ASSISTED=true        # AI-assisted development disclosure (default: on)
...
**Enabled by default.** All templates include an additional disclosure
paragraph in the Notice section:
...
This applies to all template types (CIS, DISA, and Plain). To omit it — for a
repository developed without AI assistance — pass `--no-ai-assisted` or set
`REPO_MINDER_AI_ASSISTED=false`.
